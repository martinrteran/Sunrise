...
#ifdef __cplusplus
char SSD1306_PutS(std::string str, FontDef_t* Font, SSD1306_COLOR_t color) {
	/* Write characters */
	for(char each:str){
		if(SSD1306_Putc(each, Font, color)!=each){
			return each;
		}
	}
...
char SSD1306_Putc(const char ch, FontDef_t* Font, SSD1306_COLOR_t color) {
	uint32_t i, b, j;
